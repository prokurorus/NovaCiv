  ...
  "netlify/functions/news-cron.js"(exports2) {
    ...
    exports2.handler = async (event) => {
      ...
      try {
        ...
        if (FIREBASE_DB_URL2) {
          try {
            const topicsUrl = `${FIREBASE_DB_URL2}/forum/topics.json`;
            const topicsResp = await fetch(topicsUrl);
            if (topicsResp.ok) {
              const topicsData = await topicsResp.json();
        ...
        const FIREBASE_DB_URL2 = process.env.FIREBASE_DB_URL;
        const stateByLang = {};
        for (const lang of ["ru", "en", "de"]) {
          try {
            if (FIREBASE_DB_URL2) {
              const stateUrl = `${FIREBASE_DB_URL2}/newsMeta/state_${lang}.json`;
...
`.trim();
                ...
                if (FIREBASE_DB_URL2) {
                  try {
                    const saveUrl = `${FIREBASE_DB_URL2}/forum/topics.json`;
                    const saveResp = await fetch(saveUrl, {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify(translatedTopic)
          ...
          try {
            ...
            if (telegramResult && telegramResult.ok && telegramResult.result) {
              ...
              const updateUrl = `${FIREBASE_DB_URL2}/forum/topics/${safeTopicId}.json`;
              const updateData = {
                posted: true,
                postedAt,
                telegram: {
...
var fetchNewsHandler = require_fetch_news().handler;
var newsCronHandler = require_news_cron().handler;
var domovoyAutoPostHandler = require_domovoy_auto_post().handler;

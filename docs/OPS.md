...
## ✅ Проверка Admin/Public Domovoy разделения

**Ручная проверка после изменений:**

### Админ-панель (/admin)

1. Откройте `/admin` в свежем режиме инкогнито
2. Должно автоматически открыться модальное окно Identity
3. Войдите через GitHub (если провайдер включён)
4. После входа роль admin должна быть обнаружена
5. Отправьте сообщение → функция должна вернуть JSON `{ok: true, reply: "..."}`
6. Выйдите → пользователь должен быть полностью разлогинен (после обновления остаётся разлогинен) и модал снова открывается

### Публичный помощник

1. На любой публичной странице откройте помощника (AssistantWidget)
2. Задайте вопрос: "как настроить netlify identity?"
3. Должен вернуться ответ: "Этот вопрос доступен только в /admin."
4. Проверьте, что помощник не отвечает на админские темы

#!/bin/bash
# Полная проверка nova-ops-agent на сервере
# Использование: ssh root@77.42.36.198 "bash -s" < scripts/check-ops-agent-complete.sh
# Или на сервере: bash scripts/check-ops-agent-complete.sh
...
if echo "$PM2_LIST" | grep -q "$APP_NAME"; then
  echo "✅ Процесс $APP_NAME присутствует в PM2 list"
fi

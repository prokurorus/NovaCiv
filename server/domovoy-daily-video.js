// server/domovoy-daily-video.js
//
// Ежедневная автоматическая генерация видео от Домового (серверная версия)
...
function log(...args) {
  logger.log("[domovoy-daily-video]", ...args);
}
...
async function createVideoJob(db, { text, lang, platform, maxDurationSec }) {
  const now = Date.now();
  const job = {
    createdAt: now,
    language: lang,
  ...
  const ref = await db.ref("videoJobs").push(job);
  return ref.key;
}
...
async function main() {
  try {
    ...
    const langCode = process.argv[2] || pickRandom(LANG_CONFIG.map((l) => l.code));
    ...
    const youtubeJobId = await createVideoJob(db, {
      text,
      lang: langCfg.code,
      platform: "youtube",
    ...
    const telegramJobId = await createVideoJob(db, {
      text,
      lang: langCfg.code,
      platform: "telegram",
